...
//  + Growth Insights: Career Summary
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        setStats(prev => ({
          ...prev,
          shortlisted: apps.filter(a => ['interviewing', 'hired'].includes(a.status)).length || 5,
          applicationRate: apps.length || 24,
          timeline
        }));
    ...
    <div style={S.container}>
      ...
      <div style={S.header}>
        <div>
          <div style={S.badge}>
            <div style={S.dot} />
            <p style={S.badgeText}>Career Insights</p>
          </div>
          <h1 style={S.title}>
            Growth <span style={{ color: "#CBD5E1", fontWeight: 300 }}>Insights</span>
          </h1>
        </div>
      ...
      <div style={S.grid}>
        ...
        <div className="premium-glass-card" style={S.card}>
          ...
          <h4 style={S.statLabel}>Profile Exposure</h4>
          <p style={S.statValue}>{stats.profileViews.toLocaleString()}</p>
          <div style={{ marginTop: "32px", paddingTop: "24px", borderTop: "1px solid rgba(0,0,0,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: "10px", fontWeight: 700, color: "#94A3B8" }}>Monthly Reach</span>
            <div style={{ width: "96px", height: "6px", background: "#F1F5F9", borderRadius: "3px", overflow: "hidden" }}>
              <div style={{ width: "66%", height: "100%", background: "#0055FF" }} />
            </div>
          </div>
        </div>
        ...
        <div className="premium-glass-card" style={S.card}>
          ...
          <h4 style={S.statLabel}>Total Applications</h4>
          <p style={S.statValue}>{stats.applicationRate}</p>
          <div style={{ marginTop: "32px", paddingTop: "24px", borderTop: "1px solid rgba(0,0,0,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: "10px", fontWeight: 700, color: "#94A3B8" }}>Response Rate</span>
            <div style={{ width: "96px", height: "6px", background: "#F1F5F9", borderRadius: "3px", overflow: "hidden" }}>
              <div style={{ width: "33%", height: "100%", background: "#F5A623" }} />
            </div>
          </div>
        </div>
        ...
        <div className="premium-glass-card" style={S.card}>
          ...
          <h4 style={S.statLabel}>Interview Shortlists</h4>
          <p style={S.statValue}>{stats.shortlisted}</p>
          <div style={{ marginTop: "32px", paddingTop: "24px", borderTop: "1px solid rgba(0,0,0,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: "10px", fontWeight: 700, color: "#94A3B8" }}>Profile Quality</span>
            <div style={{ width: "96px", height: "6px", background: "#F1F5F9", borderRadius: "3px", overflow: "hidden" }}>
              <div style={{ width: "85%", height: "100%", background: "#1D1D1F" }} />
            </div>
          </div>
        </div>
      ...
      <div className="premium-glass-card" style={S.chartCard}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "48px" }}>
          <h3 style={S.chartTitle}>Application Velocity</h3>
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: "#0055FF" }} />
            <span style={{ fontSize: "11px", fontWeight: 700, color: "#94A3B8", textTransform: "uppercase", letterSpacing: "1px" }}>Sent Applications</span>
          </div>
        </div>
        <div style={{ height: "400px", width: "100%" }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={stats.timeline} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              ...
              />
              <Bar dataKey="applications" fill="#0055FF" radius={[8, 8, 0, 0]} barSize={40} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
