// src/pages/candidate/Messages.js
// ═══════════════════════════════════════════════════════
//  Candidate inbox + chat
// ═══════════════════════════════════════════════════════
    ...
    <div style={S.container}>
      ...
      <div style={S.sidebar}>
        <div style={S.sideHeader}>
          <h2 style={S.sideTitle}>Direct Messages</h2>
          <p style={S.sideSub}>{conversations.length} Active Threads</p>
        </div>
        ...
        <div style={S.convList}>
          {loading ? (
            ...
            <div style={{ padding: "64px 32px", textAlign: "center" }}>
              ...
              <p style={{ color: "#94A3B8", fontWeight: 700, fontSize: "14px" }}>No messages yet.</p>
            </div>
          ) : (
            conversations.map(conv => {
              ...
              return (
                <div key={conv.id} onClick={() => handleSelect(conv)} style={S.convItem(isActive)}>
                  <div style={S.avatar}>
                    {initial}
                    {conv.myUnread > 0 && <div style={S.unreadBadge}>{conv.myUnread > 9 ? "9+" : conv.myUnread}</div>}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: "13px", color: "#64748B", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{conv.lastMessage || "No messages yet"}</div>
                  </div>
                </div>
              );
            })
      ...
      <div style={S.chatArea}>
        {selected ? (
          ...
          <div style={S.emptyChat}>
            ...
            <h3 style={{ fontSize: "20px", fontWeight: 900, color: "#1D1D1F", marginBottom: "8px" }}>Inbox</h3>
            <p style={{ color: "#94A3B8", fontWeight: 600, maxWidth: "280px", lineHeight: 1.5 }}>Select a thread to continue your professional dialogue.</p>
          </div>
        )}
      </div>
    </div>
